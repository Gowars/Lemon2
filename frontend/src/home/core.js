...
export function getServers(url, callback) {
    goFetch({
        ...
        callback: (res) => {
            ...
            const servers = atob(res.body).split(/\n/).map(i => {
                if (isRocket) {
                    if (i.startsWith('vmess://')) {
                        ...
                        const [security, id, add, port] = atob(pathname.slice(1)).split(/:|@/)
                ...
                if (i.startsWith('vmess://')) {
                    const setting = JSON.parse(atob(i.slice('vmess://'.length)))
                    try {
