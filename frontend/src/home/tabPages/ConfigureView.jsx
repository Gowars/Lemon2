...
const form2pac = v => {
    return JSON.stringify(
        v.split(/\n/).map(i => i.trim().replace(/^domain:/i, '')).filter(i => i), null, 4)
}
...
export function ConfigureView() {
    ...
    return <div className={cx('flex1 pp30', S.preview, 'scroller')}>
        <div className="ui-flex">
            <span className="flex1"></span>
            <Button className='mb10' onClick={submit}>Save</Button>
        </div>
        <Form value={state} onChange={v => setState(v)} noRoot>
            ...
            <FieldView title="Local Sock Listen Port:">
                <Input name="socksPort" placeholder="" className={S.flex1} />
            </FieldView>
            ...
            <FieldView title="Remote Pac Url" tips=''>
                <Input name="remotePacUrl" placeholder="http://127.0.0.1:80/proxy.js" className={S.flex1} />
            </FieldView>
        </Form>
    </div>
}
