...
function geNetStat(output = '') {
    ...
    const [bytes_in, bytes_out ] = line.split(/\s+/).slice(-2).map(i => Number(i) || 0)
    return {
        bytes_in,
        bytes_out,
    }
...
export function NetStatView() {
    ...
    useEffect(() => {
        return interval(() => {
            callGo('get-v2ray-stats').then(code => {
                ...
                if (prev) {
                    speed.per_in = humanSize(Math.max(info.bytes_in - prev.bytes_in, 0) || 0)
                    speed.per_out = humanSize(Math.max(info.bytes_out - prev.bytes_out, 0) || 0)
                }
    ...
    return <div className='fs11 ui-flex'>
